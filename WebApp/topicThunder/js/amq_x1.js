...
org.activemq.Amq = function() {
  ...
  var clientId = null;
	...
	var buildParams = function(msgs) {
		...
		var params = s.join('');
		if (jmsSelection)
			params += '&jmsClientId=' + jmsClientId;
		return params;
	};
	...
	var addClientId = function( data ) {
		...
		if( jmsClientId ) {
			...
			output += 'clientId='+jmsClientId;
			//output += '&JMSCorrelationId='+clientId;
		}
		return output;
	};
	...
	var getSubscribeHeaders = function() {
		...
		return jmsSelection ? {selector:_selector} : null;
	};
	...
	return {
		...
		sendMessage : function(destination, type, messageBody) {
			var message = '<message type="%s" '.replace('%s',type) + 'from="%s">'.replace('%s',jmsClientId);
		...
		addListener : function(id, destination, handler, options) {
			...
			sendJmsMessage(destination, id, 'listen', getSubscribeHeaders());
		},
